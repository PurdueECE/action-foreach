...
app.get("/:idVar/profile", function(req, res) {
    ...
    res.render("create", {page : "/" + idVar + "/create"});
});
...
app.post("/:idVar/create", function(req, res) {
    ...
    db.get("SELECT username from users where id = ?", [idVar], (error, rows) => {
        if (error) {
            ...
            if (typeof rows != 'undefined') {
                db.run("INSERT INTO posts (title, content, timestamp, votecounter, user) VALUES (?, ?, ?, ?, ?)", [title, content, timestamp.toDateString() + " " + timestamp.toTimeString(), votecounter, rows.username], (err) => {
                    if (err) {
                        console.log(err);
                    }
                })
